	...
	pd <-
		foreach( target = dt[, unique(Type)]) %do% {
			foreach( bracket = brackets) %do% {
				tryCatch({
          n <- dt[Type == target, table(Bracket == bracket, adj.P.Val < 0.05), .N]
