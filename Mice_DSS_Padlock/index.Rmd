...
params <- list(
  ...
  clustering = list(
    groups = c(Treatment="Treatment", Genotype="Genotype", Plate="substr(ID, 1, 1)"),
  ...
  model = list(
    formula   = "~ 0 + GT + .",
    formula0  = "~ 0 + . - GT",
    contrasts = c("GTwt_DSS - GTwt_Water", 
                  "GTtg_DSS - GTwt_Water",
                  "GTtg_DSS - GTtg_Water",
...
padlock <- function() {
  ...
  data$sample <- data$sample[Cohort == "RED"]
  ...
  data$modelData <- data$sample[, 
    list(GT=paste(Genotype, Treatment, sep="_") %>% as.factor %>% relevel("wt_Water"), 
      Sex)]
