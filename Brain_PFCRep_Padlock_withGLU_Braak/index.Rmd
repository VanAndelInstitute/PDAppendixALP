---
title: "Brain PFC Replicate, Padlocks"
output:
  html_document:
    toc: true
    toc_depth: 2
...
params <- list(
  ...
  model = list(
    formula = "~ 0 + Stage + .",
    formula0= "~ 0 + . - Stage",
    contrasts = c("StageEarly - StageControl",
                  "StageLate - StageControl",
                  "(StageEarly + StageLate)/2 - StageControl"),
    svRange = 1:6
  )
)
```
...
From directory "PD_Padlock_Autophagy".
...
padlock <- function() {
  ...
  data$sample$PD_Braak_Stage_Group[97] <- "PD_Braak_5_6"
  stageLevels <- unique(data$sample$PD_Braak_Stage_Group)
  stageLabels <- c("Control", "Early", "Late", "Late")


  # For consistency with older data
  data$modelData <- data$sample[, list(
    Stage = data$sample$PD_Braak_Stage_Group %>% factor(levels=stageLevels, labels=stageLabels), 
    Age, Sex, PMI, GLU=proportions_GLU)]
