...
A robust linear model implementing robust t-test between cases and controls.
...
```{r, include=TRUE}
fitBulk[, list( `Q < 0.05` = sum(adj.P.Val < 0.05),
...
merge(
  ...
  fitBulk[!duplicated(`Gene ID`), list(Gene = `Gene ID`, FC = logFC, P=P.Value)],
  by = "Gene") %>%
.[, ALP := Gene %in% genesFromALP] %>%
...
D <- model.matrix( ~ `summary diagnosis`,
   data = key[match(colnames(X), `case number`)])
