...
The goal of this data was to validate previous proteomics results. 

## Bulk tissue

```{r}
wc <- fread("./www/iTRAQ_wholeCell_human_NoNormalization_proteins.csv")
setnames(wc, "Gene", "Gene ID")
key <- fread("./www/iTRAQ_wholeCell_key.tsv")
key[, Case := as.character(Case)]

dt <- 
  suppressWarnings(
    wc[,
      list(`Accession`, `Gene ID`, Coverage = `Coverage (%)`, 
        Unique = `#Unique`,
        `3690` = as.numeric(`Intensity 3690 Control(TMT10-126)`),
        `3681` = as.numeric(`Intensity 3681 Control(TMT10-127N)`),
        `3623` = as.numeric(`Intensity 3623 PD(TMT10-127C)`),
        `3649` = as.numeric(`Intensity 3649 Control(TMT10-128N)`),
        `3823` = as.numeric(`Intensity 3823 Control(TMT10-128C)`),
        `3604` = as.numeric(`Intensity 3604 PD(TMT10-129N)`),
        `3628` = as.numeric(`Intensity 3628 Control(TMT10-129C)`),
        `3825` = as.numeric(`Intensity 3825 PD(TMT10-130N)`),
        `3531` = as.numeric(`Intensity 3531 PD(TMT10-130C)`),
        `3645` = as.numeric(`Intensity 3645 PD(TMT10-131)`))
    ])
dt <- 
  dt %>%
    .[Unique > 2] %>%
...
dt <- merge(dt, key, by.x = 'variable', by.y = 'Case')
```
...
```{r, include=TRUE}
pathways[[3]]$genes %in% dt[, unique(`Gene ID`)]  %>% mean
```
...
dt %>%
  ggplot(aes(variable, value, color = `Condition`)) + 
  geom_violin()

# PCA 
pca <- dt %>%
  reshape2::acast(`Accession` ~ variable, value.var = 'value') %>%
  as.data.frame %>%
  t %>%
  prcomp
pairs(pca$x[, 1:3], col = key[, `Condition`] %>% as.factor)
# One sample may be an ooutlier (C5), but let's keep it for now
```

A robust linear model using age, sample sex and postmortem interval as covariates was computed for each protin. 

```{r}
# Compute linear model fits
X <- reshape2::acast(dt, `Accession` ~ variable, value.var = 'value')
D <- model.matrix( ~ Condition + Age + Sex + PMI,
  data = key[match(colnames(X), Case)])
fit <- 
  lmFit(X, D, method="robust", maxit=100) %>% 
  eBayes() %>%
  topTable(coef=2, number=Inf, sort.by = "none") %>%
  as.data.table(keep.rownames=TRUE)
setnames(fit, "rn", "Accession")
fit <- 
  merge(
    fit, 
    wc[!duplicated(`Gene ID`), list(`Accession` = as.character(`Accession`), `Gene ID`)], 
    by = "Accession")
```
...
```{r, include=TRUE}
fit[, list( `Q < 0.05` = sum(adj.P.Val < 0.05),
...
proteins that were deemed significant in both datasets.

```{r, include=TRUE}
merge(
  protApx[Reliable == TRUE & abs(FC) > 0.2, list(Gene, FC)],
  fit[!duplicated(`Gene ID`) & adj.P.Val < 0.05, list(Gene = `Gene ID`, FC = logFC, P=adj.P.Val)],
  by = "Gene") %>%
.[, list(.N, cor.test(FC.x, FC.y, method="spearman") %>% broom::tidy())]
```

The ALP proteins that were significantly affected by PD in both approaches 

```{r, include=TRUE}
merge(
  protApx[Reliable == TRUE, list(Gene, FC)],
  fit[!duplicated(`Gene ID`), list(Gene = `Gene ID`, FC = logFC, P=P.Value, Q=adj.P.Val)],
  by = "Gene") %>%
.[Gene %in% genesFromALP] %>%
.[abs(FC.x) > 0.2 & Q < 0.05]
```
