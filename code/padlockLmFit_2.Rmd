...
if (all(c("logFC", "Seq") %in% colnames(fit))) {
  fit[, c(
          list(MeanFC=mean(logFC, na.rm=TRUE), NoSign = sum(adj.P.Val < 0.05, na.rm=TRUE)), 
          ftest( sign(logFC > 0), adj.P.Val < 0.05)
          ), 
    substr(Seq, 1, 2)] %>%
    kable() %>%
    kable_styling(bootstrap_options = c("striped", "hover"))  
