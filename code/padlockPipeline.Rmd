...
# # Is mean methylation in CCH loci related to diagnosis?
# lidx <- data$genome[, grepl("CC", Seq)]
# # lidx <- data$genome[, grepl("CG", Type)]
# data$beta[lidx, ] %>% 
#   matrixStats::colMedians(na.rm=TRUE) %>% 
#   # colMeans(., na.rm=TRUE) %>%
#   data.table(data$modelData, MeanCCH=.) %>%
#   glm(Group ~ ., data = ., family=binomial(link=logit)) %>%
#   summary
...
# m <- beta2m(data$beta, fixInfinity = 0.001)
# m %>% 
#   # matrixStats::colMedians(na.rm=TRUE) %>% 
#   colMeans(., na.rm=TRUE) %>%
#   data.table(data$modelData, MeanCCH=.) %>%
...
# # subtract CHH effect
# # Fit model to each locus and take residuals?
# # Quantile normalize beta values?

# # And then how to test? 
#   # Fit the model to subset of loci, check for enrichment?
