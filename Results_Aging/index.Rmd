...
apax2 <- fread("../Appendix_AgeAcceleration_Padlock_AgeMatched/www/m6_Appendix_AgeAcceleration_Padlock_AgeMatched.csv")
...
**[Additional file 12](`r url`) DNA methylation changes in the aging appendix of healthy individuals and PD patients.**
...
```{r, include=TRUE, results='asis'}
value1 <- apax[p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, mean(abs(B.DiagnosisControl.Age), na.rm=TRUE)] %>%
  `*`(100) %>% round(digits=2)
value2 <- apax[p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, sum(p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, na.rm=TRUE), Gene] %>% .[, mean(V1)] %>% 
  round(digits = 1)
glue("- {value2} differentially methylated sites per affected ALP gene with {value1}% average methylation change per year")
```
...
foo <- function(dt, A, B, title) {
  ...
  broom::tidy() %>% 
  mutate(Test = title) %>%
  dplyr::select(Test, estimate, p.value, conf.low, conf.high)
}
...
If we look into the genes that are aging in both, control and PD samples, can we see some trend in directionality?

```{r, include=TRUE}
dt[, table(IsHealthyAging=IsHealthyAging==TRUE, IsPDAging = IsPDAging == TRUE)]

dt[IsHealthyAging==TRUE & IsPDAging == TRUE, 
  table(sign(HealthyAging), sign(PDAging))] %>%
  fisher.test

dt[IsHealthyAging==TRUE & IsPDAging == TRUE, 
  t.test(abs(HealthyAging), abs(PDAging), paired=TRUE)]


dt[IsHealthyAging==TRUE & IsPDAging == TRUE, 
  list(mean(abs(HealthyAging)), mean(abs(PDAging)))]


```

```{r}
# How can I test whether the agreement is more likely at some point?

dt[IsHealthyAging & IsPDAging,
  table(sign(HealthyAging), sign(PDAging))] %>% 
  fisher.test()
...
**[Additional file 13](`r url`) DNA methylation changes in aging prefrontal cortex neurons of healthy individuals and PD patients.**
...
```{r, include=TRUE, results='asis'}
value1 <- pfcax[p.adjust(P.GroupCTRL.Age, "fdr") < 0.05, mean(abs(B.GroupCTRL.Age), na.rm=TRUE)] %>%
  `*`(100) %>% round(digits=2)
value2 <- pfcax[p.adjust(P.GroupCTRL.Age, "fdr") < 0.05, sum(p.adjust(P.GroupCTRL.Age, "fdr") < 0.05, na.rm=TRUE), Gene] %>% .[, mean(V1)] %>% 
  round(digits = 1)
glue("- {value2} differentially methylated sites per affected ALP gene with {value1}% average methylation change per year")
```
...
```{r, include=TRUE, results='asis'}
value1 <- ofbax[p.adjust(P.GroupCTRL.Age, "fdr") < 0.05, mean(abs(B.GroupCTRL.Age), na.rm=TRUE)] %>%
  `*`(100) %>% round(digits=2)
value2 <- ofbax[p.adjust(P.GroupCTRL.Age, "fdr") < 0.05, sum(p.adjust(P.GroupCTRL.Age, "fdr") < 0.05, na.rm=TRUE), Gene] %>% .[, mean(V1)] %>% 
  round(digits = 1)
glue("- {value2} differentially methylated sites per affected ALP gene with {value1}% average methylation change per year")
```
...
**[Additional file 14](`r url`) DNA methylation changes in aging olfactory bulb of healthy individuals and PD patients.**


# Overlap of aging loci/genes across tissues

Here we take the loci that show aging in different tissues of control samples. We try to evaluate how loci and genes overlap across tissues. 
...
A <- apax %>%
  ...
  .[, list(ApxAging = wmean(FC, -log(P)), IsApxAging = any(Q < 0.05, na.rm=TRUE)), Gene]
...
B <- pfcax %>%
  ...
  .[, list(PFCAging = wmean(FC, -log(P)), IsPFCAging = any(Q < 0.05, na.rm=TRUE)), Gene]
...
C <- ofbax %>%
  ...
  .[, list(OFBAging = wmean(FC, -log(P)), IsOFBAging = any(Q < 0.05, na.rm=TRUE)), Gene]

dt <- merge(merge(A, B, by = "Gene"), C, by = "Gene")

list(
  foo(dt, "IsApxAging", "IsPFCAging", title = "Appendix vs PFC"),
  foo(dt, "IsApxAging", "IsOFBAging", title = "Appendix vs OFB"),
  foo(dt, "IsPFCAging", "IsOFBAging", title = "PFC vs OFB")
  ) %>%
purrr::map(as.data.table) %>%
rbindlist %>%
kable() %>%
...
In addition to significant overlap, there is correlation of absolute aging effects

```{r, include=TRUE}
dt[IsApxAging==TRUE & IsPFCAging==TRUE, 
  cor.test(abs(ApxAging), abs(PFCAging)) %>% broom::tidy(),
  ]
```
...
**[Additional file 15](`r url`) ALP genes consistently epigenetically altered with aging in the healthy appendix and healthy prefrontal cortex neurons.**
...
X <- 
  ...
  mutate(Aging="Age effect, Control", Tissue = "Appendix")
...
X2 <- 
  merge(
    ...
    by = "ID") %>%
  setnames(c("ID", "Age", "PD")) %>%
  mutate(Aging="Age effect, PD", Tissue = "Appendix")
...
X <- 
  merge(
    ...
    by = "ID") %>%
  setnames(c("ID", "Age", "PD")) %>%
  mutate(Aging="Age effect, Control", Tissue = "PFC")
...
X2 <- 
  merge(
    ...
    by = "ID") %>%
  setnames(c("ID", "Age", "PD")) %>%
  mutate(Aging="Age effect, PD", Tissue = "PFC")
...
X <- 
  merge(
    ...
    by = "ID") %>%
  setnames(c("ID", "Age", "PD")) %>%
  mutate(Aging="Age effect, Control", Tissue = "OFB")
...
X2 <- 
  merge(
    ...
    by = "ID") %>%
  setnames(c("ID", "Age", "PD")) %>%
  mutate(Aging="Age effect, PD", Tissue = "OFB")

pd <- rbindlist(list(pd, X, X2))

textLayer <-
            pd[, cor.test(abs(Age), abs(PD)) %>% broom::tidy(), list(Tissue, Aging)] %>%
            dplyr::select(Tissue, Aging, estimate, p.value) %>%
            setDT() %>%
            .[, label := 
                  sprintf("r = $%s$, p %s %s", 
...
pd %>%
  ggplot(., aes(abs(Age), abs(PD))) + 
    ...
    xlab("Epigenetic age effect, absolute magnitude") + 
...
# Some individual gene maps

```{r}

# Gene traces
plotGeneTrace <- function(gene="SNCA", qT = 0.05, stacking = "squish", histart=90645250, hiend=90759447, ...) {
  require(biomaRt)
  require(Gviz)
  ...
  pd <- 
    apax[, list(
      Gene, ID, Chr, SNP,
      Healthy = C.DiagnosisControl.Age,
      HealthySLP = -sign(C.DiagnosisControl.Age) * log(fdr(P.DiagnosisControl.Age)),
      HealthyQ = fdr(P.DiagnosisControl.Age),
      PD = C.DiagnosisPD.LBD.Age,
      PDSLP = -sign(C.DiagnosisPD.LBD.Age) * log(fdr(P.DiagnosisPD.LBD.Age)),
      PDQ = fdr(P.DiagnosisPD.LBD.Age)
      )]
  pd <- merge(
    pd[Gene == gene],
  ...
  pd2 <- 
    pfcax[, list(
      ...
      HealthySLP = -sign(C.GroupCTRL.Age) * log(fdr(P.GroupCTRL.Age)),
      HealthyQ = fdr(P.GroupCTRL.Age),
      PD = C.GroupPD.Age,
      PDSLP = -sign(C.GroupPD.Age) * log(fdr(P.GroupPD.Age)),
  ...
  dt <- pd[, list(Chr, SNP, 
    PDvsC = -sign(logFC)*log10(adj.P.Val),
    HealthySLP, PDSLP)]
  dtr1 <-  DataTrack(chr = chr, genome = "hg19", name="Appendix, SLQ",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(PDvsC, HealthySLP, PDSLP)],
                          legend = FALSE)

  dt <- pd2[, list(Chr, SNP, 
    PDvsC = -sign(logFC)*log10(adj.P.Val),
    HealthySLP, PDSLP)]
  dtr2 <-  DataTrack(chr = chr, genome = "hg19", name="PFC, SLQ",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(PDvsC, HealthySLP, PDSLP)],
                          legend = TRUE)
  ...
  plotTracks(
      list(gtrack, biomTrack, dtr1, dtr2),
      groups = c("PD affected mean", "Age effect, Control", "Age effect, PD"),
      from = from, to = to,
      type = 'h', jitter.x = TRUE,
      baseline = c(-1.3, 1.3), col.baseline = "red", lwd.baseline = 0.25,
      lty.baseline = 2,
...
```{r, include=TRUE, fig.height=3, fig.cap="**Figure S10. PD related and aging changes at individual cytosines surrounding PD implicated genes in PD appendix and prefrontal cortex neurons. (a) SNCA. (b) LMX1B. (c) MTOR. Bar heights correspond to signed log-transformed FDR q value. Red dashed lines indicate FDR q < 0.05 threshold. Transparent red rectangles indicate the area corresponding to the gene transcript.**"}

# X11("", 7.2, 3)
suppressWarnings(plotGeneTrace(gene="SNCA", qT=1, histart=90645250, hiend=90759447))
suppressWarnings(plotGeneTrace("LMX1B", qT = 0.2, histart=129376722, hiend=129463311))
suppressWarnings(plotGeneTrace(gene="MTOR", qT = 0.2, histart=11166588, hiend=11322608))
```

# Appendix, age restricted, padlocks

Here we look into appendix aging analysis where appendix samples were restricted to 62 years or older.


```{r, include=TRUE, results='asis'}
n <- apax2[, sum(p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, na.rm=TRUE)]
glue("- Number of significant loci {n}")
n <- apax2[p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, length(unique(Gene))]
glue("- Number of genes affected {n}")
r <- apad$key[Diagnosis == "Control", range(Age) %>% paste(collapse=":")]
glue("- Age range of healthy controls {r}")
```
...
```{r, include=TRUE, results='asis'}
value1 <- apax2[p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, mean(abs(B.DiagnosisControl.Age), na.rm=TRUE)] %>%
  `*`(100) %>% round(digits=2)
value2 <- apax2[p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, sum(p.adjust(P.DiagnosisControl.Age, "fdr") < 0.05, na.rm=TRUE), Gene] %>% .[, mean(V1)] %>% 
  round(digits = 1)
glue("- {value2} differentially methylated sites per affected ALP gene with {value1}% average methylation change per year")
```

## Hyper-methylation of aging loci
...
# touch()
analysis <- "Appendix_AgeAcceleration_Padlock_AgeMatched"
prefix <- "m6"
t <- 0.05

dtlist <- cache(
  foo = foo, fname = glue("enrichment_{analysis}_AgeMatched.RDS"), verbose = TRUE,
  analysis = analysis, prefix= prefix, annotations = annotations,
  t = t
  )
```
...
```{r, include=TRUE, fig.cap="**Figure S18. Genomic elements enriched with differentially methylated cytosines in the healthy aging appendix using only samples older than 62 years.**"}
ggarrange(myplots[[1]], myplots[[2]], myplots[[3]],
  ncol = 3, nrow = 1, labels = LETTERS[1:3],
  widths = c(4, 7, 3),
  common.legend = TRUE)
...
```{r}
url <- "../Appendix_AgeAcceleration_Padlock_AgeMatched/www/m6_Appendix_AgeAcceleration_Padlock_AgeMatched.csv"
stopifnot(file.exists(url))
```

**[Additional file 20](`r url`) DNA methylation changes in the aging appendix of healthy individuals and PD patients restricted to those above 62 years of age.**
