...
plotGeneTrace <- function(gene="SNCA", qT = 0.05, stacking = "squish", histart=90645250, hiend=90759447, ...) {
  ...
  dt <- pd[, list(Chr, SNP, Q=adj.P.Val, -sign(logFC)*log10(adj.P.Val))]
  dt[, V5 := ifelse(Q < 0.05, V4, NA)]
  dtr1 <-  DataTrack(chr = chr, genome = "hg19", name="Appendix PD/C, SLP",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(V4)],
                          legend = TRUE)    

  dt <- pd[, list(Chr, SNP, -sign(Healthy)*log10(HealthyQ))]
  dtr2 <-  DataTrack(chr = chr, genome = "hg19", name="Appendix healthy aging, SLP",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(V3)],
                          legend = TRUE)    
  dt <- pd[, list(Chr, SNP, -sign(PD)*log10(PDQ))]
  dtr3 <-  DataTrack(chr = chr, genome = "hg19", name="Appendix PD aging, SLP",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(V3)],
                          legend = TRUE)

  dt <- pd2[, list(Chr, SNP, -sign(logFC)*log10(adj.P.Val))]
  dtr4 <-  DataTrack(chr = chr, genome = "hg19", name="PFC PD/C, SLP",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(V3)],
                          legend = TRUE)    
  dt <- pd2[, list(Chr, SNP, -sign(Healthy)*log10(HealthyQ))]
  dtr5 <-  DataTrack(chr = chr, genome = "hg19", name="PFC healthy aging, SLP",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(V3)],
                          legend = TRUE)    
  dt <- pd[, list(Chr, SNP, -sign(PD)*log10(PDQ))]
  dtr6 <-  DataTrack(chr = chr, genome = "hg19", name="PFC PD aging, SLP",
                          start = dt$SNP, end = dt$SNP, 
                          data = dt[, list(V3)],
                          legend = TRUE)

  plotTracks(
      list(gtrack, biomTrack, dtr1, dtr2, dtr3, dtr4, dtr5, dtr6),
