...
params <- list(
  ...
  model         = list(
    formula     = "~ 0 + Diagnosis + Diagnosis:Age + Sex + PMI",
    formula0    = "~ 0 + Diagnosis + Age + Sex + PMI",
    contrasts = c("DiagnosisControl.Age", "DiagnosisPD.LBD.Age", "DiagnosisControl.Age - DiagnosisPD.LBD.Age"),
    svRange     = 1:6
  )
)
...
# Balance the age across diagnostic groups
sidx <- data$sample[, Cohort != "Aging" & Age >= 62]
data <- shrinkList(data, sidx, nrow(data$sample))
```
