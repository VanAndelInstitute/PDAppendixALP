---
title: "Brain PFC, Padlocks, CG only"
output:
  html_document:
    toc: true
    toc_depth: 2
...
From directory "PD_Padlock_Appendix_PFC".
...
padlock <- function() {
  ...
  # Data directory
  d <- "../input/PD_Padlock_Appendix_PFC/"

  # Sample key
  data$sample <- fread(file.path(d, "Covar_Outliers"))
  data$sample[24, ID := 3561]

  # read the beta values
  data$beta <- fread(file.path(d, "CXreport_Sort_30xCov_BetaValues_Target_Outliers_30na_rm0_SNP.tsv"))  
  ...
  # Fix sample names 
  colnames(data$counts) <- gsub("PFC_.*_CON", "", colnames(data$counts))
  colnames(data$counts) <- gsub("PFC_.*_PD", "", colnames(data$counts))
  colnames(data$counts) <- gsub("_", "", colnames(data$counts))
  stopifnot(all(colnames(data$counts) == data$sample$ID))

  colnames(data$beta) <- gsub("PFC_.*_CON", "", colnames(data$beta))
  colnames(data$beta) <- gsub("PFC_.*_PD", "", colnames(data$beta))
  colnames(data$beta) <- gsub("_", "", colnames(data$beta))
